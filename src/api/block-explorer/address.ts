...
export const useGetAddressTransactions = (address: string, params?: any) => {
  return useFetch<Transaction[]>(getUrl() + '/' + address + '/transactions', params, { keepPreviousData: true, retry: false });
}
...
export const useGetAddressTotalRewards = (address: string) => {
  return useFetch<{ totalAmount: number; isValidator: boolean }>(
    REACT_APP_DAG_EXPLORER_API_URL + '/' +  + '/addresses/' + address + '/rewards'
  );
}
